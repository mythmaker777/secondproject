...
from instagram_parser import parse_zip, parse_html_file, merge_and_compute
...
HOW_TO_EXPORT = (
    ...
    "10. Leave format as default (HTML) and tap *Create files*\n\n"
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "You can also send the individual HTML files: "
        "`followers_1.html` then `following.html`\n\n"
        "_/cancel to stop._",
        parse_mode="Markdown",
    )
...
async def show_paywall(result: dict, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "✅ *Analysis complete!*\n\n"
        "👥 You follow *" + str(result["following_count"]) + "* accounts\n"
        "🤝 *" + str(result["followers_count"]) + "* follow you back\n"
        "🔒 Your non-follower results are ready\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "🔓 *Unlock your full results for SGD " + str(config.PAYMENT_AMOUNT) + "*\n\n"
...
async def _handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if fname.endswith(".zip"):
        ...
        parsed = parse_html_file(file_bytes, doc.file_name or fname)
        ...
        context.user_data["html_" + file_type] = parsed
        await status.delete()

        if context.user_data.get("html_" + other_type):
            result = merge_and_compute(
                context.user_data["html_followers"],
                context.user_data["html_following"],
            )
            if not result["success"]:
                await update.message.reply_text(
                    "❌ " + result["error"] + "\n\nType /start to restart.",
            ...
            needed = "following.html" if file_type == "followers" else "followers_1.html"
            await update.message.reply_text(
...
def main():
    ...
    user_conv = ConversationHandler(
        ...
        states={
            ...
            # pay_method button is handled INSIDE the conversation here
            # so the state correctly advances to AWAITING_PAYMENT_REF
