...
Instagram will email you a download link — usually within 10–30 minutes. Come back when it arrives and send the ZIP file here!"""

WELCOME = """👋 Welcome to *Respectagram* 🔍
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Send the `.zip` file from the Instagram email.\n\n"
        "_/cancel to stop._",
        parse_mode="Markdown",
    )
...
async def _handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Individual HTML file ─────────────────────────────────
        ...
        context.user_data[f"html_{file_type}"] = parsed
        await status.delete()

        if context.user_data.get(f"html_{other_type}"):
        ...
        await status.edit_text(
            ...
            "You can also send the individual `.html` files: `followers_1.html` and `following.html`."
        )
        return AWAITING_FILE
